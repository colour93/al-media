...
export function VideoSidebarCard({ video }: VideoSidebarCardProps) {
  ...
  const tagPreview = actors.length === 0 && creators.length === 0 ? tags.slice(0, 2) : [];
  ...
  return (
    <Box
      ...
      sx={{
        display: 'grid',
        gridTemplateColumns: { xs: '108px minmax(0,1fr)', sm: '126px minmax(0,1fr)' },
        gap: 1,
        textDecoration: 'none',
        color: 'inherit',
        borderRadius: 1.5,
    ...
    >
      ...
      <Box sx={{ minWidth: 0, overflow: 'hidden', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
        <Typography
          variant="body2"
          fontWeight={600}
          title={video.title}
        ...
        <Box sx={{ mt: 0.25, display: 'inline-flex', alignItems: 'center', gap: 0.5, color: 'text.secondary' }}>
          <Eye size={12} />
        ...
        <Box sx={{ mt: 0.35, display: 'flex', flexWrap: 'wrap', gap: 0.35, alignItems: 'center' }}>
          {actorPreview.map((a) => (
